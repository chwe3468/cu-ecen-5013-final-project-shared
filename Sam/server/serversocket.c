...
/*       //Other method
         ...
         char byte_send;
         while(fread(&byte_send, sizeof(char), sizeof(char), fp))
         {

           send(accepted, (const void *)&byte_send, sizeof(char), 0);
         }
...
int main(int argc, char * argv[])
{
	...
	while(1)
	{
		...
		if(signal_exit == true)
		{
			...
			while(head != NULL)
			{
				/*Wait for lock so thread isn't killed while holding mutex*/
				pthread_mutex_lock(&head->nodeLock);
				pthread_mutex_destroy(&head->nodeLock);
				if(pthread_join(head->thread_id, NULL))
				{
					err = errno;
					syslog(LOG_ERR, "Unable to join thread %ld, error %s\n", head->thread_id, strerror(err));
				}
